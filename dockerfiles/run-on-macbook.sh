...
if [ -n "$MODULE_CHECK" ]; then
    ...
    if echo "$MODULE_CHECK" | grep -q "google"; then
        ...
        if [ -n "$GOOGLE_PROJECT_ID" ]; then
            ...
            echo ""
            echo "    To configure (without restarting container):"
            echo "    1. Edit supervisor config inside container:"
            echo "       docker exec -it $CONTAINER_NAME bash"
            echo "       vi /etc/supervisor/conf.d/freeswitch-google.conf"
            echo ""
            echo "    2. Update the environment line with your credentials:"
            echo "       environment=LD_LIBRARY_PATH=\"/usr/local/lib:/usr/local/freeswitch/lib\",GOOGLE_APPLICATION_CREDENTIALS=\"/etc/freeswitch/google-creds.json\",GOOGLE_PROJECT_ID=\"your-actual-project-id\",GCP_LOCATION=\"us-central1\""
            echo ""
            echo "    3. Restart FreeSWITCH process (not container):"
            echo "       supervisorctl restart freeswitch"
            echo "       exit"
            echo ""
            echo "    4. Mount credentials file (if not already mounted):"
            echo "       docker cp /path/to/creds.json $CONTAINER_NAME:/etc/freeswitch/google-creds.json"
            echo ""
            echo "    5. Then use the module:"
            echo "       docker exec -it $CONTAINER_NAME fs_cli"
            echo "       freeswitch@internal> uuid_google_transcribev2 <uuid> start en-US"
        fi
    fi
