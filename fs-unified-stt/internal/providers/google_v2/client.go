...
func (c *Client) StartStream(ctx context.Context, cfg interfaces.StreamConfig) (interfaces.Stream, error) {
	// Handle endpoint: "global" uses the default endpoint, regional uses {region}-speech.googleapis.com
	var endpoint string
...
func (c *Client) getOrCreateRecognizer(ctx context.Context, client *speech.Client, cfg interfaces.StreamConfig) (string, error) {
	...
	// Use recognizer ID directly without language suffix
	// The recognizer can support multiple languages in its config
	recognizerID := c.cfg.GCPRecognizerID
	...
	if err != nil {
		st, _ := status.FromError(err)
		// Only ignore AlreadyExists - for other errors, log and still try to use the recognizer
		if st.Code().String() == "AlreadyExists" {
			c.log.Info("Recognizer already exists, using it", "recognizer", full)
		} else {
			c.log.Error("CreateRecognizer failed",
				"recognizer", full,
				"grpc_code", st.Code().String(),
				"grpc_msg", st.Message(),
			)
			// Don't cache on error - let it try again next time
			return full, nil
		}
		c.recMu.Lock()
		c.recognizers[key] = full
