...
type Meta struct {
	...
	// Legacy Flat Fields (Compatible with FreeSWITCH Dialplan)
	LegacyCallID      string `json:"call_id"`
	LegacyCallerName  string `json:"caller_name"`
	LegacyCallerNum   string `json:"caller_number"`
	LegacyCalleeName  string `json:"callee_name"`
	LegacyCalleeNum   string `json:"callee_number"`
}
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.log.Error("failed to read metadata", "err", err)
		return
	}

	var meta Meta
	if err := json.Unmarshal(payload, &meta); err != nil {
		s.log.Error("invalid metadata json", "err", err)
		return
	}

	// metadata normalization (support flat legacy fields)
	if meta.SIPCallID == "" && meta.LegacyCallID != "" {
		meta.SIPCallID = meta.LegacyCallID
	}
	if meta.Caller.Name == "" {
		meta.Caller.Name = meta.LegacyCallerName
	}
	if meta.Caller.Phone == "" {
		meta.Caller.Phone = meta.LegacyCallerNum
	}
	if meta.Callee.Name == "" {
		meta.Callee.Name = meta.LegacyCalleeName
	}
	if meta.Callee.Phone == "" {
		meta.Callee.Phone = meta.LegacyCalleeNum
	}

	// Validation
	if meta.UUID == "" || meta.SIPCallID == "" || meta.SampleRate <= 0 || meta.Channels <= 0 {
		s.log.Error("missing required metadata fields", "meta", meta)
		return
	}
	if meta.Lang == "" {
		meta.Lang = "en-US"
	...
	// Mutex for safe concurrent writes to WebSocket
	var wsMu sync.Mutex
	
	// Callback to send results back to WebSocket client (only for mock/load testing)
	var onResult func(result interfaces.Result)
	if providerName == "mock" {
		onResult = func(result interfaces.Result) {
