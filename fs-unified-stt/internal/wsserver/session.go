...
type SessionDeps struct {
	...
	OnResult   func(result interfaces.Result) // Callback to send results to WS client
}
...
func (s *Session) Start() error {
	...
	// Publish session-started event to Pusher
	startEvent := map[string]interface{}{
		"uuid":        s.meta.UUID,
		"sip_call_id": s.meta.SIPCallID,
		"lang":        s.meta.Lang,
		"channels":    s.meta.Channels,
		"caller":      s.meta.Caller,
		"callee":      s.meta.Callee,
		"ts_ms":       time.Now().UnixMilli(),
	}
	if err := s.pub.Publish(s.meta.SIPCallID, "session-started", startEvent); err != nil {
		s.log.Warn("pusher session-started publish failed", "uuid", s.meta.UUID, "err", err)
	}
...
func (s *Session) Stop() {
	...
	if s.stream != nil {
		s.stream.Close()
		// Do not set s.stream = nil to avoid race conditions in goroutines
	}
	s.log.Info("session stopped", "uuid", s.meta.UUID, "sip_call_id", s.meta.SIPCallID)

	// Publish session-stopped event to Pusher
	stopEvent := map[string]interface{}{
		"uuid":        s.meta.UUID,
		"sip_call_id": s.meta.SIPCallID,
		"ts_ms":       time.Now().UnixMilli(),
	}
	if err := s.pub.Publish(s.meta.SIPCallID, "session-stopped", stopEvent); err != nil {
		s.log.Warn("pusher session-stopped publish failed", "uuid", s.meta.UUID, "err", err)
	}
}
...
func (s *Session) receiverLoop() {
	...
	for {
		...
		case res, ok := <-s.stream.Results():
			...
			s.log.Info("transcription received",
				"uuid", s.meta.UUID,
				"channel", res.Channel,
			...
			// Send result back to WebSocket client (for load testing validation)
			if s.onResult != nil {
				s.onResult(res)
			}
